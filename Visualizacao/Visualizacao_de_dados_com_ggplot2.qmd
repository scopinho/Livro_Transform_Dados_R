...
::: callout-caution
Vale lembrar que nem sempre uma variável representada pr um número é quantitativa. O número da carteira de identidade é um exemplo disso. Apesar dos números ela é uma variável qualitativa.
:::
...
As variáveis qualitativas (nominais ou ordinais) podem ser representadas por gráficos para variáveis discretas no jargão do ggplot.

Veja que, como aconteceu com o combustível, podemos pegar a variável tipo do veículo (class) e gerar um gráfico de barras também.

```{r}
d + geom_bar(aes(x = class))
```

Usando o argumento da estética fill, podemos mapear uma variável (coluna). Com isto, o ggplot automaticamente criará uma legenda para os diferentes valores da varíavel. Veja:
...
Se sobrescrevermos este mapeamento para a estético do geom_bar, teremos:
...
É similar ao geom_point, porém adicoina uma variação aleatória em cada ponto.
