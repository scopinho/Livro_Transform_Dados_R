...
::: callout-important
Observe que o valor definido da cor, está fora dos parênteses da função aes(), ou seja, ela não está fazendo o mapeamento da cor com os dados, mas sim, definindo uma cor para a função da geometria (geom_point).
:::

Se quiser definir a cor no mapeamento através da estética, podemos fazê-lo de outra maneira através do argumento color dentro da função aes().

Por exemplo, vamos gerar o mesmo gráfico anterior, mas gostaríamos de incluir no mapeamento informações de estética onde a cor dos pontos estivesse mapeada com a coluna "qsec" de nossos dados mtcars:
...
Use para definir a linha utilizada no gráfico. Podemos utilizar umm número de 0 a 6 ou um nome:
...
```{r}
ggplot(mtcars)+
  geom_line(aes(x = hp, y = mpg), linetype = 4)
```
...
```{r}
ggplot(mtcars)+
  geom_line(aes(x = hp, y = mpg), lineend = "round")
```
...
```{r}
ggplot(mtcars)+
  geom_line(aes(x = hp, y = mpg), linejoin = "bevel")
```
...
Use para definir a forma. Podemos passar um número ou um nome da forma conforme abaixo:
...
ggplot(mtcars)+
  geom_point(aes(x = hp, y = mpg), shape = "square")
ggplot(mtcars)+
  geom_point(aes(x = hp, y = mpg), shape = 5)
```
